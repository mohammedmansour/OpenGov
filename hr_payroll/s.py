...
class taxes(osv.osv):
    ...
    _columns = {
            ...
            'minimum':fields.integer('Minimum_Limit',size=11, required=False, select=1),
            'maximum':fields.integer('Maximum_Limit',size=11, required=False, select=1),
...
class iti_hr_payroll_deduct_place(osv.osv):
    ...
    _columns = {
        'name':fields.text('Name', size=100, required=True, readonly=False),
    }
...
class iti_allowance_reason(osv.osv):
    ...
    _columns = {
        'name':fields.text('Allowance Reason', size=100, required=True, readonly=False),
...
class iti_deduction_reason(osv.osv):
    ...
    _columns = {
        'name': fields.text('Deduction Reason', size=100, required=True),
...
class iti_allowance(osv.osv):
    ...
    _columns = {        
        'name': fields.text('Description', size=64, required=False),
        'number': fields.text('Reference', size=64, required=False),
...
class iti_deduction(osv.osv):
    ...
    _columns = {        
        ...
        'employee_ids': fields.many2one('hr.employee', 'Employee'),
        #'emp_payslip_id': fields.one2many('hr.payslip','emp_deduct_id', 'Payslip'),
        'by_employee_ids': fields.many2one('hr.employee', 'By'),
        'deduction_amount': fields.float('Amount', required=True, help="The Employee Deducted Amount."),
...
class damgha(osv.osv):
    ...
    _columns = {
            ...
            'class':fields.integer('Class',size=11, required=False, select=1),
            'percentage':fields.float('Percentage', digits=None, required=False, select=1),
            'minimum':fields.integer('Minimum_Limit',size=11, required=False, select=1),
            'maximum':fields.integer('Maximum_Limit',size=11, required=False, select=1),
...
class hr_payslip(osv.osv):
    ...
    def _get_deductions(self, cr, uid, ids, field_names, arg=None, context=None):
        result = {}
        
        return result
    
    def _get_allowances(self, cr, uid, ids, field_names, arg=None, context=None):
        result = {}
    ...
    _columns = {
        ...
        'state': fields.selection([
            ...
            \n* If the payslip is under verification, the state is \'Waiting\'. \
             \n* If the payslip is Stoped, the state is \'Stoped\'. \
...
#        'line_ids': fields.one2many('hr.payslip.line', 'slip_id', 'Payslip Line', required=False, readonly=True, states={'draft': [('readonly', False)]}),
        ...
        'contract_id': fields.one2many('hr.contract', 'Contract', required=False, readonly=True, states={'draft': [('readonly', False)]}),
        ...
        #'emp_deduct_id': fields.many2one('iti.deduction','Deductions'),        
