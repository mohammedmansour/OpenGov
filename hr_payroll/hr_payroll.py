...
###Added By Kotb
class taxes(osv.osv):
    _name = 'hr.payroll.taxes'
    _columns = {
            'class':fields.integer('Class',size=11, required=False, select=1),
            'percentage':fields.float('Percentage', digits=(1,4), required=False, select=1),
            'minimum':fields.integer('MinimumLimit',size=11, required=False, select=1),
            'maximum':fields.integer('MaximumLimit',size=11, required=False, select=1),
...
## Added By Mansour
#####################################################################
class iti_hr_payroll_deduct_place(osv.osv):
    _name='iti.hr.payroll.deduct.palce'
    _columns = {
        'name':fields.char('Name', size=256, required=True, readonly=False),
    }

iti_hr_payroll_deduct_place()
########################### Allownces and Decuction Types ############
class iti_allowance_reason(osv.osv):
    _name = 'iti.allowance.reason'
    _description = 'Allowances Reasons'
    
    _columns = {
        'name':fields.char('Allowance Reason', size=256, required=True, readonly=False),
        'iti_allowance_id': fields.one2many('iti.allowance', 'allowance_reason_ids' , 'Allowances'),    
        
    }
    
iti_allowance_reason()     

class iti_deduction_reason(osv.osv):
    _name = 'iti.deduction.reason'    
    _description = 'Deductions Reasons'
    _columns = {
        'name': fields.char('Deduction Reason', size=256, required=True),
        'iti_deduction_id': fields.one2many('iti.deduction', 'deduction_reason_ids' , 'Deductions'),    
    }

iti_deduction_reason()
#################### Allowances And Deductions #######################

class iti_allowance(osv.osv):
    '''
    Allowances
    '''

    _name = 'iti.allowance'
    _description = 'Add New Allowances'        
    _columns = {        
        'name': fields.char('Description', size=64, required=False, readonly=True),
        'number': fields.char('Reference', size=64, required=False, readonly=True),
        'employee_ids': fields.many2one('hr.employee', 'Employee'),
        'by_employee_ids': fields.many2one('hr.employee', 'By'),
        'payslip_id': fields.many2one('hr.payslip', 'Pay Slip', required=True, ondelete='cascade', select=True),
        'allowance_amount': fields.float('Amount', required=True, help="The Extra Employee Added Allowance."),
        'allowance_reason_ids': fields.many2one('iti.allowance.reason', 'Allowance Reason'),
        'date_from': fields.date('Date From', readonly=True, required=True),
        'date_to': fields.date('Date To', readonly=True, required=True),
        'taxable': fields.boolean('Taxabl'),
    }
    _defaults = {
...
#'contract_ids': fields.many2one('hr.contract', 'Contract'),        
...
class iti_deduction(osv.osv):
    '''
    Deductions
    '''

    _name = 'iti.deduction'
    _description = 'Add New Deduction'        
    _columns = {        
        'name': fields.char('Description', size=64, required=False, readonly=False),
        'number': fields.char('Reference', size=64, required=False, readonly=False),
        'employee_ids': fields.many2one('hr.employee', 'Employee'),
        'by_employee_ids': fields.many2one('hr.employee', 'By'),
        'deduction_amount': fields.float('Amount', required=True, help="The Employee Deducted Amount.", ondelete='cascade', select=True),
        'deduction_reason_ids': fields.many2one('iti.deduction.reason', 'Deduction Reason'),
        'date_from': fields.date('Date From', readonly=True, required=True),
        'date_to': fields.date('Date To', readonly=True, required=True),
        'payslip_id': fields.many2one('hr.payslip', 'Pay Slip', required=True, ondelete='cascade', select=True),
...
iti_hr_payroll_deduct_place()
##                      <!--  Added By Mohamed kotb Damgha   -->
class damgha(osv.osv):
    _name = 'hr.payroll.damgha'
    _columns = {
            'name':fields.char('Name',size=100,required=False),
            'class':fields.integer('Class',size=11, required=False, select=1),
            'percentage':fields.float('Percentage', digits=(1,6), required=False, select=1),
            'minimum':fields.integer('MinimumLimit',size=11, required=False, select=1),
            'maximum':fields.integer('MaximumLimit',size=11, required=False, select=1),
...
damgha()
##                  <!-- End of Kotb Add-->
class annual_raise(osv.osv):
    
    _name = 'hr.payroll.annual.raise'
    _description = 'Annual Raise'
    _columns = {
        'year':fields.integer('Year', required=True),
        'raise':fields.float('Raise', required=True),
        'added': fields.boolean('Added'),
    }
    
    def raise_update(self,cr,uid,ids=None,context=None):
        current_year = date.today().year
        print  current_year
        #self.pool.get(’sale.order’).write(cr, uid, ids, {’added’:’true’})
        rec = cr.execute('UPDATE hr_payroll_annual_raise SET added=TRUE \
        WHERE year<='+str(current_year-5)+';', )
        print rec
...
class hr_payslip(osv.osv):
    ...
    def _get_lines_salary_rule_category(self, cr, uid, ids, field_names, arg=None, context=None):
        ...
        return result
## method to get the allowances for the current payslip     
    def get_allowances(self, cr, uid, ids, field_name, arg, context):
        res = {}
  
        for i in self.browse(cr, uid, ids, context):
            cr.execute("SELECT * from iti_allowance where payslip_id="+str(i.id)+" AND taxable=True")
            allow = cr.dictfetchall()
        
            temp =0
          
            for d in allow:
                temp += d.get('allowance_amount','')
                
            res[i.id] = temp 
            
        return res
## method to get the deductions for the current payslip         
    def get_deductions(self, cr, uid, ids, field_name, arg, context):
        res = {}
  
        for i in self.browse(cr, uid, ids, context):
            cr.execute("SELECT * from iti_deduction where payslip_id="+str(i.id)+"")
            ded = cr.dictfetchall()
        
            temp =0
          
            for d in ded:
                temp += d.get('deduction_amount','')
    ...
    _columns = {
        ...
        #'details_of_deductions': fields.function(_get_deductions, method=True, type='one2many', relation='iti.deduction', string='Details by Salary Rule Category'),
        'emp_deduction_id': fields.one2many('iti.deduction', 'payslip_id', 'Payslip Deductions', required=False, readonly=True, states={'draft': [('readonly', False)]}),
        #'details_of_allowances': fields.function(_get_allowances, method=True, type='one2many', relation='iti.allowance', string='Details by Salary Rule Category'),
        'emp_allowance_id': fields.one2many('iti.allowance', 'payslip_id', 'Payslip Allowances', required=False, readonly=True, states={'draft': [('readonly', False)]}),
        #'emp_allowance_id': fields.one2many('iti.allowance','employee_ids','Employee On Pilot Allowances'),
        'allowances': fields.function(get_allowances, type='float', method=True,string='Allowances'),
        'deductions': fields.function(get_deductions, type='float', method=True,string='Deductions'),
